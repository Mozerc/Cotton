---
description: lipoxygenase12 ; Zm00001eb134020 ; Zea mays
meta_title:
title: ZmLOX12;lox12
categories: [Zea mays]
tags: [ jasmonate,defense response,defense,jasmonate response ]
---

## Gene Details:
- **Gene ID:**	[Zm00001eb134020]()
- **Gene Symbol:** ZmLOX12;lox12
- **Gene Name:** lipoxygenase12
- **Genome:** [Zm-B73-REFERENCE-NAM-5.0]()
- **Species:** *Zea mays*

## Functional Description:
   - The Novel Monocot-Specific 9-Lipoxygenase **Zm**lox12**** Is Required to Mount an Effective Jasmonate-Mediated Defense Against Fusarium verticillioides in Maize.
   - **Zm**lox12**** is predominantly expressed in mesocotyls in which it is strongly induced in response to F. verticillioides infection. The Mutator transposon-insertional **lox12**-1 mutant is more susceptible to F. verticillioides colonization of mesocotyls, stalks, and kernels.
   - **Zm**lox12****. This gene is distantly related to known dicot LOX genes, with closest homologs found exclusively in other monocot species.

## Literature:
   - [The novel monocot-specific 9-lipoxygenase ZmLOX12 is required to mount an effective jasmonate-mediated defense against Fusarium verticillioides in maize.]( https://apsjournals.apsnet.org/doi/abs/10.1094/MPMI-06-13-0184-R?url_ver=Z39.88-2003&rfr_id=ori:rid:crossref.org&rfr_dat=cr_pub%20%200pubmed)&nbsp;&nbsp;DOI:&nbsp;&nbsp;[10.1094/MPMI-06-13-0184-R](https://apsjournals.apsnet.org/doi/abs/10.1094/MPMI-06-13-0184-R?url_ver=Z39.88-2003&rfr_id=ori:rid:crossref.org&rfr_dat=cr_pub%20%200pubmed)&nbsp;&nbsp;PMID:&nbsp;&nbsp;[25122482](https://pubmed.ncbi.nlm.nih.gov/25122482/)

## Related News:

## Gene Resources:
- **NCBI ID:** [](https://www.ncbi.nlm.nih.gov/gene/?term=)
- **UniProt accessions:** [](https://www.uniprot.org/uniprotkb//entry)

## Orthologs:

## Keywords:
jasmonate,defense response,defense,jasmonate response

## Sequences:
- **CDS Sequence:**
- **Protein Sequence:**
